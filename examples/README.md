...
The TypeScript unary gRPC transport and Inventory processing can be isolated
from the Order HTTP endpoint and Order graph with:

```bash
make grpc-only CORES=2 LOADGEN_CORES=6 VUS=256 DURATION=20s RUNS=3
```

This starts only Inventory Service and drives
`inventoryserviceapi.InventoryServiceApi/ProcessOrderItem` directly with k6.
Framework and native use the same checked-in protobuf schema, request,
connection reuse, quotas and best-run aggregation. Results are written with
the `typescript-grpc-only.*` prefix.
