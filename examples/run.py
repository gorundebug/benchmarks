    ...
    env.update(
        {
            ...
            "BENCHMARK_GRPC_PROTO_DIR": str(BENCHMARK_DIR / "proto"),
            "BENCHMARK_GRPC_TARGET": "inventoryservice:9202",
            "BENCHMARK_LOAD_SCRIPT": (
                "/scripts/load.grpc.js"
                if getattr(args, "grpc_only", False)
                else "/scripts/load.js"
            ),
...
def call_semantics_overrides(
    ...
    Runtime exporters may omit an explicit link semantic when it equals the
    service default. Generated YAML retains it, so raw token counts are not a
    stable cross-language runtime contract.
    """
...
def verify_live_service_graph_profile(
    ...
    expected = call_semantics_overrides(graph_path.read_text())
...
def write_results(results: list[dict[str, Any]], args: argparse.Namespace) -> None:
    ...
    results_markdown.write_text(
        ...
        + (
            "- Request: `gRPC inventoryserviceapi.InventoryServiceApi/ProcessOrderItem` "
            "(expected `OK`)\n"
            if args.grpc_only
            else f"- Request: `{args.method} {args.target}` (expected `{args.expected_status}`)\n"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--grpc-only",
        action="store_true",
        help="benchmark only the Inventory unary gRPC endpoint; do not start Order",
    )
    parser.add_argument(
