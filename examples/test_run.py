...
class ScenarioConfigurationTest(unittest.TestCase):
    ...
    def test_grpc_only_selects_the_grpc_load_script_and_schema(self) -> None:
        args = Namespace(
            cores=2,
            loadgen_cores=6,
            duration="20s",
        ...
        env = benchmark.environment(args, language)
        self.assertEqual(env["BENCHMARK_LOAD_SCRIPT"], "/scripts/load.grpc.js")
        self.assertEqual(env["BENCHMARK_GRPC_TARGET"], "inventoryservice:9202")
        self.assertEqual(
            env["BENCHMARK_GRPC_PROTO_DIR"],
            str(benchmark.BENCHMARK_DIR / "proto"),
        )
