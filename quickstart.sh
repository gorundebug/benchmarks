...
#   ./quickstart.sh --clone-only   # only fetch missing sibling repos, don't run
#
...
python3 "$ROOT/benchmarks/examples/run.py" --fetch-native \
  --language go-native \
  --language cpp-native \
...
echo "==> Running the benchmark (framework and native variants run sequentially)"
